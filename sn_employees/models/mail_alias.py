...
class MailAlias(models.AbstractModel):
    ...
    def _alias_check_contact_on_record(self, record, message, message_dict, alias):
        if alias.alias_contact == 'employees':
            ...
            employee = self.env['sn_employees.employee'].search([('work_email', 'ilike', email_address)], limit=1)
            if not employee:
                employee = self.env['sn_employees.employee'].search([('user_id.email', 'ilike', email_address)], limit=1)
            if not employee:
                return {
                    'error_message': 'restricted to employees',
                    'error_template': self.env.ref('sn_employees.mail_template_data_unknown_employee_email_address').body_html,
                }
            return True
